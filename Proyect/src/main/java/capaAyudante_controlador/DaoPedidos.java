...
public class DaoPedidos implements InterfacePedido{
	...
	public Producto BuscarProducto(int codigo) {
		...
		try {
			c = MysqlBDConexion.getConexion();
			String sql = "SELECT * , ACTIVO FROM PRODUCTOS WHERE COD_PRODUCTO = ?;";
			pstm = c.prepareStatement(sql);
	...
	public int AgregarPedido(Pedido ped) {
		...
		PreparedStatement pstm = null;
		ResultSet rs = null;
		System.out.println("usuario ped = " + ped.getUsuario().getIdUsuario());
		try {
			c = MysqlBDConexion.getConexion();
	...
	public List<Pedido> ListadoPedido(String inicial) {
		...
		try {
			...
			while (rs.next()) {
				...
				ped.setCliente(cli);
				usu.setIdUsuario(rs.getInt(5));
				usu.setNombre(rs.getString(10));
				usu.setApellidos(rs.getString(11));
				ped.setUsuario(usu);
				ped.setMonto(rs.getDouble(6));
				ped.setEstado(rs.getString(9));
				ped.setFecha(rs.getDate(8));
				listaPedido.add(ped);
			}
	...
	@Override
	public int AumentarStock(Pedido_detalle pedDeta) {
		int salida = -1;
		Connection c = null;
		PreparedStatement pstm = null;
		try {
			c = MysqlBDConexion.getConexion();
			String sql = "CALL SumarCantidadProducto(?, ?);";
			pstm = c.prepareStatement(sql);
			pstm.setInt(1, pedDeta.getProducto().getCodProducto());
			pstm.setInt(2, pedDeta.getCantidad());
			salida = pstm.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	@Override
	public int DescontarStock(Pedido_detalle pedDeta) {
		int salida = -1;
		Connection c = null;
		PreparedStatement pstm = null;
		try {
			c = MysqlBDConexion.getConexion();
			String sql = "CALL RestarCantidadProducto(?, ?);";
			pstm = c.prepareStatement(sql);
			pstm.setInt(1, pedDeta.getProducto().getCodProducto());
			pstm.setInt(2, pedDeta.getCantidad());
			salida = pstm.executeUpdate();
